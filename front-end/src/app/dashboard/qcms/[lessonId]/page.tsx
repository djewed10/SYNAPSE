...
const generateMockQCMs = (lessonId: string, count: number = 10): QCMWithState[] => {
  ...
  const questions = [
    ...
    'Les signes cliniques de l\'anémie ferriprive comprennent :',
    'Le bilan martial comporte :',
    'Le traitement de l\'anémie ferriprive repose sur :',
    'Les causes d\'anémie ferriprive chez l\'adulte incluent :',
    'L\'absorption intestinale du fer est favorisée par :',
  ];
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    propositions: [
      { id: `${lessonId}-${i}-a`, text: 'Fer libre Fe 2+.' },
      { id: `${lessonId}-${i}-b`, text: 'De fer de l\'hème.' },
      { id: `${lessonId}-${i}-c`, text: 'D\'apoferritine.' },
      { id: `${lessonId}-${i}-d`, text: 'De transferrine.' },
      { id: `${lessonId}-${i}-e`, text: 'D\'apotransferrine.' },
    ],
    correctAnswers: [`${lessonId}-${i}-a`, `${lessonId}-${i}-b`],
...
interface QCMsLessonPageProps {
  params: Promise<{
    lessonId: string;
  }>;
}

export default function QCMsLessonPage({ params }: QCMsLessonPageProps) {
  ...
  const layerNumber = searchParams.get('layer') || '1';
  
  // State
  const [qcms, setQcms] = useState<QCMWithState[]>(() => generateMockQCMs(lessonId, 10));
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] transition-colors duration-300">
      ...
      {/* Layer indicator */}
      <div className="px-4 md:px-6 lg:px-8 py-2 bg-[var(--surface)] border-b border-[var(--border)]">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <span className="text-sm text-[var(--text-secondary)]">
            Couche <span className="font-bold text-[var(--text-primary)]">{layerNumber}</span>
          </span>
          <span className="text-sm text-[var(--text-secondary)]">
            Leçon ID: <span className="font-mono text-xs">{lessonId}</span>
          </span>
        </div>
      </div>
